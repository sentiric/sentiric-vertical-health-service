...
RUN CGO_ENABLED=0 GOOS=linux go build \
    ...
    -o /app/bin/sentiric-vertical-health-service ./cmd/vertical-health-service
...
COPY --from=builder /app/bin/sentiric-vertical-health-service .
RUN chown appuser:appgroup ./sentiric-vertical-health-service

# GÜVENLİK: Kullanıcıyı değiştir
USER appuser

ENTRYPOINT ["./sentiric-vertical-health-service"]